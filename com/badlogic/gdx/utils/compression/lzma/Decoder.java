      ...
      while(outSize < 0L || nowPos64 < outSize) {
         ...
         if (this.m_RangeDecoder.DecodeBit(this.m_IsMatchDecoders, (state << 4) + posState) == 0) {
            ...
            if (this.m_RangeDecoder.DecodeBit(this.m_IsRepDecoders, state) == 1) {
               ...
               distance = this.m_PosSlotDecoder[Base.GetLenToPosState(len)].Decode(this.m_RangeDecoder);
               if (distance >= 4) {
                  int numDirectBits = (distance >> 1) - 1;
                  rep0 = (2 | distance & 1) << numDirectBits;
                  if (distance < 14) {
                     rep0 += BitTreeDecoder.ReverseDecode(this.m_PosDecoders, rep0 - distance - 1, this.m_RangeDecoder, numDirectBits);
                  } else {
      ...
      public int Decode(com.badlogic.gdx.utils.compression.rangecoder.Decoder rangeDecoder, int posState) throws IOException {
         if (rangeDecoder.DecodeBit(this.m_Choice, 0) == 0) {
            ...
            if (rangeDecoder.DecodeBit(this.m_Choice, 1) == 0) {
               symbolx = symbol + this.m_MidCoder[posState].Decode(rangeDecoder);
            } else {
               symbolx = symbol + 8 + this.m_HighCoder.Decode(rangeDecoder);
            }
